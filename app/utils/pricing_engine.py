...
def estimate_print_time(
    ...
    flow_rate = FLOW_RATE_CC_PER_HR.get(material_slug, 15)
...
def calculate_price(
    ...
    adjusted_manufacturing_cost = round(
        base_manufacturing_cost * complexity_multiplier * orientation_multiplier, 2
    )
    ...
    subtotal = round(
        adjusted_manufacturing_cost + platform_fee + packaging_fee + delivery_fee, 2
    )
